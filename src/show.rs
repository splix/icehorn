...
//!
//! Keeping the show family in its own module means adding a new inspector
//! (snapshots, schema, partitions…) stays a single-file change.
...
use crate::cli::{ShowCommand, ShowVersionArgs};
use crate::config::S3Config;
use crate::s3_url::S3Location;

pub async fn run(cmd: ShowCommand) -> Result<()> {
    match cmd {
        ShowCommand::Version(args) => show_version(args).await,
    }
}

async fn show_version(args: ShowVersionArgs) -> Result<()> {
...
impl MetadataFile {
    ...
    fn parse(path: &ObjPath) -> Option<Self> {
        ...
        Some(MetadataFile { version, uuid })
    }
}
...
mod tests {
    ...
    fn rejects_manifest_list() {
        // snap-<id>-<attempt>-<uuid>.avro — lives next to metadata.json files
        assert!(MetadataFile::parse(&ObjPath::from(
    ...
    fn rejects_manifest_file() {
        // <commitUUID>-m<index>.avro — starts with a UUID, not a version.
        assert!(MetadataFile::parse(&ObjPath::from(
    ...
    fn rejects_hadoop_style_name() {
        // Hadoop catalog uses `v<N>.metadata.json` without a UUID and with a
        // non-5-digit version — we currently only handle the standard layout.
    ...
    async fn find_latest_skips_interleaved_manifests_and_stops_at_non_digit() {
        ...
        let seed = [
            ...
            // Files past the digit range — we rely on these being skipped
            // by the early-stop, not by MetadataFile::parse.
